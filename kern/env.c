...
env_setup_vm(struct Env *e)
{
	...
	cprintf("env_pgdir address %x\n",e->env_pgdir);
	p->pp_ref+=1;
	pde_t * env_pgs = e->env_pgdir;

	for (int i = PDX(UTOP); i < NPDENTRIES; ++i) {
		env_pgs[i] = kern_pgdir[i];
	}
...
region_alloc(struct Env *e, void *va, size_t len)
{
    ...
    while(va_start < va_end){
        //how to check if current virtual address is allocated or not? Using page_lookup()
        struct PageInfo * new_page = page_alloc(0);
        if(!new_page){
            panic("Allocation Attempt Failed!");
        } else{
            // Pages should be writable by user and kernel = PTE_W | PTE_U
            page_insert(e->env_pgdir, new_page, va_start, PTE_W | PTE_U);
        }
        va_start += PGSIZE;
    }
